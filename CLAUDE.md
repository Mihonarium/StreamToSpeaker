...
- **AirPlay — GROUND TRUTH (packet capture, 2026-07-02, iTunes-for-Windows → Sonos SYMFONISK):** a working sender uses **classic plaintext RAOP** with this device — `POST /auth-setup` (0x01+X25519) as the FIRST request (⚠️ NO `OPTIONS`+Apple-Challenge opener — this firmware stalls on it), then ANNOUNCE (standard SDP, no encryption lines)/SETUP/RECORD, realtime UDP PT96 352-spf compressed ALAC, `0xD4` sync @1 Hz with **latency 88200** (2 s, = node_airtunes2's `2*sampling_rate`) sent simultaneously with first audio, and `0xD2`/`0xD3` timing. **No HomeKit pairing, no AP2 plists, no PTP, no buffered mode, no event channel, no /feedback** — advertising bit 40/41 does NOT mean a receiver wants them from senders; Apple's own Windows client ignores all of it. The AP2/PTP/buffered stack we built (below) is real and matches every reference implementation, but the Sonos withholds *playback* on those paths (accepts everything, hoards ~130 kB, never plays — receiver-side gate unknown). Sonos-class receivers now route RAOP-first with the auth-setup opener; AP2 is for HomePods/AP2-only devices. Useful reference the user found: `ciderapp/node_airtunes2` (Cider's sender — confirms the capture byte-for-byte). Raw capture insights: receiver Server header `AirTunes/366.0`; sync anchor = `next_rtptime − 88200`; iTunes sends SET_PARAMETER volume + progress after RECORD, OPTIONS only as keepalive.
