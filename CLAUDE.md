...
- **AirPlay — GROUND TRUTH (re-analysed 2026-07-03 from the `itunes_full.pcapng` the user provided, iTunes-for-Windows 12.13.10 → Sonos SYMFONISK "Picture frame", fw `AirTunes/366.0`, TXT `et=0,4 cn=0,1 sf=0x4`):** ⚠️ **CORRECTION to the earlier note — iTunes does NOT use plaintext. It uses classic RAOP with `et=4` MFi *encryption*.** The earlier "plaintext, no encryption lines" reading was wrong (it was probably why every plaintext build stayed silent). Verified byte-for-byte:
    - **Two TCP connections.** A short probe connection first: `POST /auth-setup` (0x01+X25519) → 200 (713 B = 32 B receiver X25519 pubkey + 4 B certlen + PKCS#7 MFi cert + 4 B siglen + 64 B sig), then plain `OPTIONS *` (⚠️ **no** Apple-Challenge header) → 200. Then the streaming connection: its OWN `POST /auth-setup` (CSeq 1) → `ANNOUNCE` (CSeq 2) → `SETUP` → `RECORD` → `SET_PARAMETER`.
    - **ANNOUNCE SDP is encrypted:** `a=mfiaeskey:<16 B, base64-nopad>` + `a=aesiv:<16 B>`, standard `a=fmtp:96 352 0 16 40 10 14 2 255 0 0 44100`. **`o=` = sender IP, `c=` = RECEIVER IP** (NOT sender — a prior build regressed `c=` to sender; fixed). URI = `rtsp://<sender-ip>/<u32 session id>` (sender IP, 32-bit id — a 64-bit id / receiver-IP URI silently stalls SETUP).
    - **`mfiaeskey` is a *wrapped* key, NOT the raw audio key.** Decrypting the captured realtime audio (UDP PT96, compressed ALAC, AES-128-CBC, whole 16-B blocks only) with the raw `mfiaeskey` yields uniform-random ALAC element tags in *every* cipher mode (CBC/CTR/ECB/OFB/CFB) — i.e. garbage. So the receiver recovers the real audio key by unwrapping `mfiaeskey` with the `/auth-setup` X25519 shared secret. **The exact wrap is unverifiable from the capture** (the ECDH shared secret needs a private key that isn't on the wire) and **no open-source project implements et=4** (OwnTone/libraop/shairport/node_airtunes2 all do plaintext or RSA-only; for et=4 devices they do a *throwaway* auth-setup then stream **plaintext** — but our own capture shows this Sonos fw 200s a plaintext ANNOUNCE and then **stalls SETUP**, so plaintext is not enough here).
    - **Why encryption, not something else, is the SETUP gate:** our old plaintext session (same capture set) got ANNOUNCE→200 then SETUP→(no response), with the *same* URI that ANNOUNCE accepted. The lone material diff vs iTunes's working SETUP is the `mfiaeskey`/encrypted audio.
    - Sync `0xD4` @1 Hz, **latency 88200** (field2 = current RTP write head, field1 = field2−88200, NTP wall-clock mid); `0xD2`/`0xD3` timing echo. First sync at RECORD. These all already match in `timing.rs`.
    - **IMPLEMENTED 2026-07-03 (`crypto.rs` `MfiKey`, `rtsp.rs`, `session.rs`): et=4 wrapped-key path.** Random audio key/IV encrypt the audio (AES-128-CBC, same as RSA path); the key is wrapped as `mfiaeskey = audio_key XOR AES-ECB(KEK, KIV)` with `KEK=SHA1("AES-KEY"‖shared)[..16]`, `KIV=SHA1("AES-IV"‖shared)[..16]` (openairplay MFi-SAP derivation, single-block AES-CTR). `session.rs` prefers `Cipher::Mfi` when the device advertises et=4 and auth-setup succeeded; else plaintext/RSA. **UNVERIFIED against hardware** — the wrap mode/KEK is the best-grounded guess, not capture-confirmed. If it's still silent, the audio-key wrap is the thing to vary (try ECB `AES-ECB(KEK, audio_key)`, or CBC, or a different KEK); getting past SETUP at all (vs the plaintext stall) is the diagnostic signal that `mfiaeskey` presence is what the receiver wanted. Useful reference: `ciderapp/node_airtunes2` matches the RTP/sync/timing byte-for-byte (but uses a hardcoded RSA key, not MFi).
- **AirPlay — implemented; remaining work + caveats.** The AirPlay sender lives in `service/src/airplay/` and is wired into discovery + the picker (each device is classified `RaopLegacy` vs `AirPlay2` via the `_airplay._tcp` `features`/`ft` bits; HomePods — `model AudioAccessory*` — always route to AirPlay 2). Two paths:
    - **AirPlay 1 / RAOP** — unencrypted (`et=0`), Apple-RSA (`et=1`), or **MFi (`et=4`, wrapped key — see GROUND TRUTH above; preferred for et=4 devices like the Sonos)**: AirPort Express, shairport-sync, Sonos/Apple-TV in legacy mode. `rtsp.rs` / `rtp.rs` / `alac.rs` / `crypto.rs` / `timing.rs` / `session.rs`.
