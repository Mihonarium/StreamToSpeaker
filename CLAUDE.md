...
- **AirPlay — implemented; remaining work + caveats.** The AirPlay sender lives in `service/src/airplay/` and is wired into discovery + the picker (each device is classified `RaopLegacy` vs `AirPlay2` via the `_airplay._tcp` `features`/`ft` bits; HomePods — `model AudioAccessory*` — always route to AirPlay 2). Two paths:
  ...
  **Verified vs. not:** the crypto core is unit-tested (SRP against the RFC 5054 vector; TLV8/HKDF/channel-cipher/audio-seal round-trips); the PTP packet codec + offset math and the resend buffer are unit-tested too. The *live* pairing + stream — and **especially the PTP handshake** — have **not** been validated against a real HomePod; that needs the user's hardware. Status of the formerly-deferred items:
    - **PTP timing — IMPLEMENTED (`ap2_ptp.rs`), UNVERIFIED vs hardware.** Receivers advertising `SupportsPTP` (feature bit 41) now negotiate `timingProtocol: PTP`: SETUP carries `timingPeerInfo`, we send `SETPEERS`, run a PTP follower on UDP 319/320 (announce at priority1=250, yield via BMCA to the HomePod grandmaster, exchange Sync/Follow_Up/Delay_Req/Delay_Resp, maintain a smoothed master−local offset), and anchor the `0xD4` sync packet to the shared PTP clock. Non-PTP receivers keep the NTP path. **Open risk:** the realtime+PTP anchor semantics are *inferred*, not confirmed — if a HomePod rejects realtime+PTP it most likely wants the *buffered/AAC* stream instead (see below). Refs: OwnTone `airplay.c`, `lmcgartland/airplay2-rs`.
